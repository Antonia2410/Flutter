...
import 'package:carrito_compra/model/Item.dart';
...
class Carrito with ChangeNotifier {
  ...
  double get subTotal {
    ...
    items.forEach((key,elementos) 
      =>total += elementos.precio *elementos.cantidad);
    return total;
  ...
  void agregarItem(String producto_id, String nombre, double precio, String unidad, String imagen, int cantidad){
    if(_items.containsKey(producto_id)){
      _items.update(producto_id, (old) => Item(
        id: old.id,
        nombre: old.nombre,
        precio: old.precio,
        unidad: old.unidad,
